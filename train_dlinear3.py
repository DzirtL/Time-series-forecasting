...
if __name__ == "__main__":
    ...
    save_dir = r"C:\Users\artem\Documents\DeepLearning\task_3\runs"
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)

    experiments = [
        {"input_slice": 12, "stride": 4},
        {"input_slice": 24, "stride": 24},
        {"input_slice": 48, "stride": 12}
    ]

    for exp_cfg in experiments:
        exp_suffix = f"slice{exp_cfg['input_slice']}_stride{exp_cfg['stride']}"
        print(f"\n>>> НАЧИНАЮ ЭКСПЕРИМЕНТ: {exp_suffix}")

        cfg = {
            "input_dim": 1,
            "input_slice": exp_cfg["input_slice"],
            ...
            "device": "cuda:0",
            "inp_seq_len": hist_len,
            "outp_seq_len": pred_len
        }
        ...
        if model_type == "Transformer_MT_S1":
            cfg={"input_dim":1, "input_slice": exp_cfg["input_slice"], "stride": exp_cfg["stride"], "embed_dim":128,"num_heads":8,"out_dim":1,"enc_numlayers":1,"dec_numlayers":1,"dropout":0.1,"temporal_encoding":False,"device":"cuda:0","inp_seq_len":hist_len,"outp_seq_len":pred_len}
        ...
        train_schedule = {
            "loss": [], "best_loss": None,
            "spath": os.path.join(save_dir, f"{model_type}_{exp_suffix}_train.pytorch"),
            "model": model, "device": device
        }
        test_schedule = {
            "loss": [], "best_loss": None,
            "spath": os.path.join(save_dir, f"{model_type}_{exp_suffix}_test.pytorch"),
            "model": model, "device": device
        }
        ...
        for epoch in range(50):
            ...
            print(f"### [{exp_suffix}] epoch: {epoch} train stat: {train_stat}")
            model_scheduler(train_schedule,epoch,train_stat["loss"])

            model.eval()
            test_stat = process_dataset(test_loader,"test")
            print(f"### [{exp_suffix}] epoch: {epoch} test stat: {test_stat}")
            model_scheduler(test_schedule,epoch,test_stat["loss"])
            
            model.eval()
            eval_stat = process_dataset(test_loader,"infer")
            print(f"### [{exp_suffix}] epoch: {epoch} infer stat: {eval_stat}")

        train_save = {k: v for k, v in train_schedule.items() if k != 'model'}
        test_save = {k: v for k, v in test_schedule.items() if k != 'model'}
        
        with open(os.path.join(save_dir, f"{model_type}_{exp_suffix}_traffic_train.json"), "w") as f:
            json.dump(train_save, f)
        with open(os.path.join(save_dir, f"{model_type}_{exp_suffix}_traffic_test.json"), "w") as f:
            json.dump(test_save, f)

        plt.figure()
        plt.plot(train_schedule["loss"], label="train")
        plt.plot(test_schedule["loss"], label="test")
        plt.title(f"Loss {exp_suffix}")
        plt.legend()
        plt.savefig(os.path.join(save_dir, f"plot_{model_type}_{exp_suffix}_traffic_results.png"))
        plt.close()

        print(f"Best rez on test for {exp_suffix}:", test_schedule["best_loss"])
