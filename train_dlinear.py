...
if ipython is not None:
    ...
    # Set autoreload mode (e.g., 2 to reload all modules)
...
#simple time series forecasting dataset
class TrafficDataset(data.Dataset):
    def __init__(self,path,junction,seq_len,pred_len,mode="train"):
        ...
        self.data = df["Vehicles"][sel]
        self.data_dateno= df["Date_no"][sel]
        self.data_month = df["Month"][sel]   
        self.data_day= df["Weekday"][sel]
        self.data_hour = df["Hour"][sel]



        self.data = torch.tensor(self.data)
        self.data_dateno = torch.tensor(self.data_dateno)
        self.data_month = torch.tensor(self.data_month)
        self.data_day = torch.tensor(self.data_day)
        self.data_hour = torch.tensor(self.data_hour)


        self.data = self.data


        #self.data = torch.tensor(generate_data(len(self.data))).float()
        ...
        if mode=="train":
            self.data_idx = torch.arange(0,len(self.data)*0.8).long()
        else:
            self.data_idx = torch.arange(int(len(self.data)*0.8),len(self.data)-1-seq_len-pred_len).long()
            

        return

        fig,ax = plt.subplots(2,1,sharex=True)
        ax[0].plot(df["DateTime"][sel],self.data)
        ax[1].plot(df["DateTime"][sel],df["Weekday"][sel])
        plt.show()
    ...
    def __getitem__(self,idx_):
        ...
        times = torch.cat([self.data_month[start:end],self.data_dateno[start:end],self.data_day[start:end],self.data_hour[start:end]]).reshape(4,-1)

        #tokens = (tokens-torch.mean(tokens[:,:self.seq_len],dim=-1,keepdim=True))/torch.std(tokens[:,:self.seq_len],dim=-1,keepdim=True)*0.1
        

        times = times.permute(1,0)
        tokens = tokens.float().permute(1,0)
        #
        return tokens[:self.seq_len],tokens[self.seq_len-1:-1],tokens[self.seq_len:],times[:self.seq_len],times[self.seq_len-1:-1]
...
def process_dataset(loader,mode,lr_scheduler=None):
    ...
    for pos,data in enumerate(tqdm(iter(loader))):
        ...
        if pos==1 and pos>0:
            ...
            # plt.show()
            plt.savefig(f"{model_type}_plot_{mode}.png")
            plt.close()
...
if __name__ == "__main__":
    ...
    dataset_train = ETTDataset("ETTh1.csv", hist_len, pred_len, "train")
    train_loader = data.DataLoader(dataset_train, batch_size=8, shuffle=True) 

    dataset_test = ETTDataset("ETTh1.csv", hist_len, pred_len, "test")
    ...
    train_schedule = {"loss":[],"best_loss":None,"spath":"runs/"+model_type+"_train.pytorch","model":model,"device":device}
    test_schedule = {"loss":[],"best_loss":None,"spath":"runs/"+model_type+"_test.pytorch","model":model,"device":device}
